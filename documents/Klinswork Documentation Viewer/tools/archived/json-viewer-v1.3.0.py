...
provides generic tree/raw inspection, document-sidecar previews, and a catalog
browser for catalogs containing an ``entries`` array.

Recommended layout:

    work-update-catalog/
    ├── tools/
    │   └── json-viewer-v1.3.0.py
    ├── catalogs/
    ├── sidecars/
    ├── summaries/
    ├── templates/
    ├── workflows/
    ├── archived/
    └── open-json-viewer-v1.3.0.bat

Recommended launch command from the repository root:

    py -3 "%~dp0tools\json-viewer-v1.3.0.py" --folder "%~dp0"
...
APP_VERSION = "1.3.0"
BUILD_ID = "2026-08-01-repository-catalog-browser"
HOST = "127.0.0.1"
...
<head>
  ...
  <title>Local JSON Explorer v1.3.0</title>
  <style>
    :root {
...
<div class="app" id="app">
  <aside class="sidebar" id="sidebar">
    <div class="sidebar-header">
      ...
      <p class="subtle runtime-info" id="runtimeInfo">Version 1.3.0 · loading runtime identity…</p>
...
(() => {
  ...
  async function loadAbout() {
    try {
      ...
      els.runtimeInfo.textContent = "Version 1.3.0 · runtime identity unavailable";
    }
  }
  ...
  function renderCurrentView() {
    ...
    els.searchInput.placeholder = state.view === "catalog"
      ? "Search catalog titles, projects, topics, or text"
  ...
  function looksLikeCatalog(data) {
    return isPlainObject(data) && Array.isArray(data.entries) && (isPlainObject(data.catalog) || data.entries.some(looksLikeDocumentSidecar));
  }
  ...
  function buildCatalogModel(data) {
    ...
    const catalogRecord = isPlainObject(data.catalog) ? data.catalog : {};
  ...
  function renderCatalog() {
    ...
    header.innerHTML = `
      <p class="catalog-eyebrow">Catalog browser</p>
      ...
      <div class="catalog-stats">
        ...
        ${model.generated ? `<span class="preview-chip">Generated: ${escapeHtml(model.generated)}</span>` : ""}
    ...
    controls.innerHTML = `
      <div class="catalog-control"><label for="catalogProject">Project</label><select id="catalogProject"><option value="">All projects</option></select></div>
      <div class="catalog-control"><label for="catalogType">Document type</label><select id="catalogType"><option value="">All types</option></select></div>
      <div class="catalog-control"><label for="catalogYear">Year</label><select id="catalogYear"><option value="">All years</option></select></div>
      <div class="catalog-control"><label for="catalogSort">Sort</label><select id="catalogSort"><option value="newest">Newest first</option><option value="oldest">Oldest first</option><option value="title">Title</option><option value="catalog">Catalog order</option></select></div>
    `;
  ...
  function renderCatalogResults() {
    ...
    list.innerHTML = "";
    if (!entries.length) {
      list.innerHTML = '<div class="catalog-empty">No catalog entries match the current search and filters.</div>';
      els.status.textContent = "No catalog entries match.";
      return;
    }
    ...
    for (const entry of entries) {
      ...
      for (const [label, value] of [["Date", entry.date], ["Project", entry.project], ["Type", entry.type]]) {
        if (!value) continue;
  ...
  function renderPreview() {
    ...
    els.previewPanelHeading.textContent = state.previewContext === "catalog" ? "Catalog entry preview" : "Document preview";
    ...
    eyebrow.textContent = state.previewContext === "catalog" ? "Catalog entry preview" : "Document sidecar preview";
    ...
    footer.appendChild(document.createTextNode("This abbreviated view is generated from structured JSON. The linked HTML document remains the complete publication."));
    const missing = [];
  ...
  function updatePreviewButton() {
    ...
    els.previewButton.title = available ? "Show or hide the document preview" : "No document preview is available for this selection";
  }
