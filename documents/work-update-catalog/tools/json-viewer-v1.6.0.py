...
Recommended launch command from the repository root:
    ...
    py -3 "%~dp0tools\json-viewer-v1.6.0.py" --folder "%~dp0"
...
APP_VERSION = "1.6.0"
BUILD_ID = "2026-08-05-pst-sp-study-guide-renderer"
HOST = "127.0.0.1"
...
<head>
  ...
  <title>Local JSON Explorer v1.6.0</title>
  <style>
    :root {
    ...
    * { box-sizing: border-box; }
    ...
    .pst-preview-sections { padding: .85rem 1.12rem 1.1rem; }
    .pst-tabs { position: sticky; top: -.85rem; z-index: 3; display: flex; gap: .32rem; overflow-x: auto; margin: -.85rem -1.12rem .8rem; padding: .68rem 1.12rem; background: color-mix(in srgb, var(--panel) 94%, transparent); border-bottom: 1px solid var(--line); backdrop-filter: blur(8px); }
    .pst-tab { flex: 0 0 auto; padding: .36rem .55rem; font-size: .7rem; font-weight: 750; }
    .pst-tab[aria-selected="true"] { color: var(--accent-strong); background: var(--accent-soft); border-color: var(--accent); }
    .pst-panel[hidden] { display: none !important; }
    .pst-summary-grid, .pst-detail-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: .5rem; }
    .pst-stat-grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: .45rem; margin: .65rem 0; }
    .pst-stat { padding: .62rem; text-align: center; background: var(--panel-2); border: 1px solid var(--line); border-radius: 9px; }
    .pst-stat strong { display: block; color: var(--accent-strong); font-size: 1.08rem; }
    .pst-stat span { display: block; margin-top: .15rem; color: var(--muted); font-size: .62rem; }
    .pst-card-list { display: grid; gap: .58rem; }
    details.pst-card { overflow: hidden; background: var(--panel-2); border: 1px solid var(--line); border-radius: 10px; }
    details.pst-card > summary { display: grid; grid-template-columns: auto minmax(0, 1fr) auto; gap: .55rem; align-items: start; padding: .68rem .72rem; cursor: pointer; list-style: none; }
    details.pst-card > summary::-webkit-details-marker { display: none; }
    details.pst-card > summary::before { content: "▸"; color: var(--accent); font-weight: 850; transition: transform .1s ease; }
    details.pst-card[open] > summary::before { transform: rotate(90deg); }
    .pst-card-title strong { display: block; color: var(--accent-strong); font-size: .84rem; line-height: 1.3; }
    .pst-card-title span { display: block; margin-top: .15rem; color: var(--muted); font-size: .69rem; line-height: 1.4; }
    .pst-card-badge { padding: .15rem .36rem; color: var(--muted); background: var(--panel); border: 1px solid var(--line); border-radius: 999px; font-size: .59rem; white-space: nowrap; }
    .pst-card-body { padding: .68rem .74rem .78rem; background: var(--panel); border-top: 1px solid var(--line); }
    .pst-card-body p { margin: 0 0 .58rem; color: var(--text); font-size: .79rem; line-height: 1.48; }
    .pst-detail { min-width: 0; padding: .5rem .55rem; background: var(--panel-2); border: 1px solid var(--line); border-radius: 8px; }
    .pst-detail h4 { margin: 0 0 .28rem; color: var(--accent-strong); font-size: .62rem; letter-spacing: .04em; text-transform: uppercase; }
    .pst-detail ul { margin: 0; padding-left: 1rem; }
    .pst-detail li { margin: .22rem 0; color: var(--text); font-size: .69rem; line-height: 1.38; }
    .pst-source-button { padding: .16rem .35rem; color: var(--muted); background: transparent; border-color: transparent; font-family: var(--mono); font-size: .59rem; }
    .pst-source-button:hover { color: var(--accent-strong); background: var(--accent-soft); border-color: var(--accent); }
    .pst-filter-row { display: grid; grid-template-columns: minmax(0, 1fr) auto; gap: .45rem; margin-bottom: .6rem; }
    .pst-filter-row input, .pst-filter-row select { width: 100%; padding: .46rem .55rem; color: var(--text); background: var(--panel-2); border: 1px solid var(--line); border-radius: 8px; }
    .pst-visual-row { display: grid; grid-template-columns: minmax(0, 1fr) auto; gap: .55rem; padding: .62rem .68rem; background: var(--panel-2); border: 1px solid var(--line); border-radius: 9px; }
    .pst-visual-row strong { display: block; font-size: .78rem; }
    .pst-visual-row span { display: block; margin-top: .15rem; color: var(--muted); font-size: .67rem; }
    @media (max-width: 560px) { .pst-summary-grid, .pst-detail-grid { grid-template-columns: 1fr; } .pst-stat-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
...
<div class="app" id="app">
  <aside class="sidebar" id="sidebar">
    <div class="sidebar-header">
      ...
      <p class="subtle runtime-info" id="runtimeInfo">Version 1.6.0 · loading runtime identity…</p>
...
(() => {
  ...
  async function loadAbout() {
    try {
      ...
      els.runtimeInfo.textContent = "Version 1.6.0 · runtime identity unavailable";
    }
  }
  ...
  function buildSidecarPreview(data, basePath = "") {
    ...
    return {
      ...
      pstSpContent: pickPreviewField(data, ["pstSpContent"], basePath),
      sourceData: data,
      basePath,
    };
  }
  ...
  function isPstSpPreview(model) {
    if (!model) return false;
    const mode = fieldText(model.previewMode).toLowerCase();
    const type = fieldText(model.documentType).toLowerCase();
    return mode === "pst-sp-study-guide" || type === "pst-sp-study-guide" || Boolean(model.pstSpContent && isPlainObject(model.pstSpContent.value));
  }

  function pstArray(value) { return Array.isArray(value) ? value : []; }
  function pstObject(value) { return isPlainObject(value) ? value : {}; }
  function pstText(value, maxLength = 900) { return compactText(value, maxLength); }

  function makePstSourceButton(path) {
    if (!path) return null;
    const button = document.createElement("button");
    button.type = "button";
    button.className = "pst-source-button";
    button.textContent = "JSON";
    button.title = `Reveal ${path}`;
    button.addEventListener("click", event => { event.stopPropagation(); revealJsonPath(path); });
    return button;
  }

  function appendPstListBlock(container, title, values) {
    const items = pstArray(values).filter(isMeaningful);
    if (!items.length) return;
    const block = document.createElement("section"); block.className = "pst-detail";
    const heading = document.createElement("h4"); heading.textContent = title;
    const list = document.createElement("ul");
    for (const value of items) { const item = document.createElement("li"); item.textContent = pstText(value, 500); list.appendChild(item); }
    block.append(heading, list); container.appendChild(block);
  }

  function appendPstTextBlock(container, title, value) {
    if (!isMeaningful(value)) return;
    const block = document.createElement("section"); block.className = "pst-detail";
    const heading = document.createElement("h4"); heading.textContent = title;
    const paragraph = document.createElement("p"); paragraph.style.margin = "0"; paragraph.style.fontSize = ".7rem"; paragraph.textContent = pstText(value, 800);
    block.append(heading, paragraph); container.appendChild(block);
  }

  function makePstCard(record) {
    const details = document.createElement("details"); details.className = "pst-card";
    const summary = document.createElement("summary");
    const heading = document.createElement("div"); heading.className = "pst-card-title";
    const strong = document.createElement("strong"); strong.textContent = record.title || "Untitled record"; heading.appendChild(strong);
    if (record.subtitle) { const span = document.createElement("span"); span.textContent = record.subtitle; heading.appendChild(span); }
    summary.appendChild(heading);
    if (record.badge) { const badge = document.createElement("span"); badge.className = "pst-card-badge"; badge.textContent = record.badge; summary.appendChild(badge); }
    const source = makePstSourceButton(record.path); if (source) summary.appendChild(source);
    details.appendChild(summary);
    const body = document.createElement("div"); body.className = "pst-card-body";
    if (record.purpose) { const paragraph = document.createElement("p"); paragraph.textContent = record.purpose; body.appendChild(paragraph); }
    const grid = document.createElement("div"); grid.className = "pst-detail-grid";
    for (const block of record.blocks || []) { if (block.kind === "list") appendPstListBlock(grid, block.title, block.value); else appendPstTextBlock(grid, block.title, block.value); }
    if (grid.children.length) body.appendChild(grid);
    details.appendChild(body); return details;
  }

  function makePstPanel(id, label) { const panel = document.createElement("section"); panel.className = "pst-panel"; panel.dataset.tabId = id; panel.setAttribute("aria-label", label); return panel; }

  function makePstHeading(title, description = "") {
    const wrapper = document.createElement("div"); wrapper.style.marginBottom = ".65rem";
    const heading = document.createElement("h3"); heading.style.margin = "0"; heading.style.color = "var(--accent-strong)"; heading.style.fontSize = ".92rem"; heading.textContent = title; wrapper.appendChild(heading);
    if (description) { const p = document.createElement("p"); p.style.margin = ".28rem 0 0"; p.style.color = "var(--muted)"; p.style.fontSize = ".76rem"; p.style.lineHeight = "1.45"; p.textContent = description; wrapper.appendChild(p); }
    return wrapper;
  }

  function renderPstOverview(content, model) {
    const panel = makePstPanel("overview", "Overview");
    const program = pstObject(content.program), source = pstObject(content.sourcePacket), curriculum = pstObject(content.curriculumModel), coverage = pstObject(content.coverage);
    panel.appendChild(makePstHeading("Program overview", pstText(program.programPurpose || model.summary?.value || "", 1200)));
    const stats = document.createElement("div"); stats.className = "pst-stat-grid";
    for (const [value, label] of [[pstArray(content.toolkits).length,"Toolkits"],[pstArray(content.exercises).length,"Exercises"],[pstArray(content.measures).length,"Measures"],[pstArray(content.visualAssets).length,"Visual records"]]) { const item=document.createElement("div"); item.className="pst-stat"; const strong=document.createElement("strong"); strong.textContent=String(value); const span=document.createElement("span"); span.textContent=label; item.append(strong,span); stats.appendChild(item); }
    panel.appendChild(stats);
    const grid=document.createElement("div"); grid.className="pst-summary-grid";
    appendPstTextBlock(grid,"Source packet",program.sourcePacketTitle); appendPstTextBlock(grid,"Issuing organization",[program.issuingOrganization,program.issuingAdministration].filter(Boolean).join(" / ")); appendPstTextBlock(grid,"Coverage",source.coverageStatus || coverage.sourceCaptureStatus); appendPstTextBlock(grid,"Editorial state",coverage.editorialIntegrationStatus); appendPstListBlock(grid,"Instructional sequence",curriculum.instructionalSequence); appendPstListBlock(grid,"Core workflow",curriculum.coreWorkflow); appendPstListBlock(grid,"Known gaps",coverage.knownGaps); appendPstListBlock(grid,"Remaining work",coverage.remainingWork); panel.appendChild(grid); return panel;
  }

  function renderPstToolkits(content, basePath) {
    const panel=makePstPanel("toolkits","Toolkits"); panel.appendChild(makePstHeading("Toolkits","Open a toolkit to review its purpose, obstacles, skills, exercises, and source coverage.")); const list=document.createElement("div"); list.className="pst-card-list";
    pstArray(content.toolkits).forEach((t,i)=>{ if(!isPlainObject(t))return; const r=pstObject(t.sourcePageRange); const pages=r.startPage?`Pages ${r.startPage}${r.endPage?`–${r.endPage}`:""}`:""; list.appendChild(makePstCard({title:t.title||t.toolkitId,subtitle:t.shortTitle&&t.shortTitle!==t.title?t.shortTitle:"",badge:[t.status,pages].filter(Boolean).join(" · "),path:`${basePath}.toolkits[${i}]`,purpose:pstText(t.purpose,1000),blocks:[{kind:"list",title:"Obstacles addressed",value:t.obstaclesAddressed},{kind:"list",title:"Skills taught",value:t.skillsTaught},{kind:"list",title:"When to use",value:t.whenToUse},{kind:"list",title:"When not to use",value:t.whenNotToUse},{kind:"list",title:"Exercises",value:t.exerciseIds},{kind:"list",title:"Related toolkits",value:t.relatedToolkitIds}]})); }); panel.appendChild(list); return panel;
  }

  function renderPstExercises(content, basePath) {
    const panel=makePstPanel("exercises","Exercises"); panel.appendChild(makePstHeading("Exercise inventory","Filter by title, type, toolkit, or intended use."));
    const controls=document.createElement("div"); controls.className="pst-filter-row"; const input=document.createElement("input"); input.type="search"; input.placeholder="Filter exercises"; const select=document.createElement("select"); select.innerHTML='<option value="">All toolkits</option>'; for(const value of [...new Set(pstArray(content.exercises).map(x=>x?.toolkitId).filter(Boolean))].sort()){const o=document.createElement("option");o.value=value;o.textContent=value;select.appendChild(o);} controls.append(input,select); panel.appendChild(controls);
    const list=document.createElement("div"); list.className="pst-card-list"; panel.appendChild(list);
    const render=()=>{const q=input.value.trim().toLowerCase(), f=select.value; list.innerHTML=""; pstArray(content.exercises).forEach((e,i)=>{if(!isPlainObject(e))return;if(q&&!JSON.stringify(e).toLowerCase().includes(q))return;if(f&&e.toolkitId!==f)return;const r=pstObject(e.sourcePageRange);const pages=r.startPage?`Pages ${r.startPage}${r.endPage?`–${r.endPage}`:""}`:"";list.appendChild(makePstCard({title:e.title||e.exerciseId,subtitle:[e.exerciseType,e.toolkitId].filter(Boolean).join(" · "),badge:[e.status,pages].filter(Boolean).join(" · "),path:`${basePath}.exercises[${i}]`,purpose:pstText(e.purpose,1000),blocks:[{kind:"list",title:"When to use",value:e.whenToUse},{kind:"text",title:"Duration",value:e.estimatedDuration},{kind:"list",title:"Preparation",value:e.preparation},{kind:"list",title:"Steps",value:e.steps},{kind:"list",title:"Practice guidance",value:e.practiceGuidance},{kind:"list",title:"Safety notes",value:e.safetyNotes}]}));}); if(!list.children.length){const empty=document.createElement("div");empty.className="catalog-empty";empty.textContent="No exercises match.";list.appendChild(empty);}}; input.addEventListener("input",render);select.addEventListener("change",render);render();return panel;
  }

  function renderPstMeasures(content, basePath) {
    const panel=makePstPanel("measures","Measures"); panel.appendChild(makePstHeading("Measures","Assessment instruments are summarized for context and are not stand-alone diagnostic tools.")); const list=document.createElement("div");list.className="pst-card-list";
    pstArray(content.measures).forEach((m,i)=>{if(!isPlainObject(m))return;const r=pstObject(m.sourcePageRange);const pages=r.startPage?`Pages ${r.startPage}${r.endPage?`–${r.endPage}`:""}`:"";list.appendChild(makePstCard({title:m.name||m.measureId,subtitle:[m.abbreviation,m.authorsOrSource,m.publicationYear].filter(Boolean).join(" · "),badge:[m.status,pages].filter(Boolean).join(" · "),path:`${basePath}.measures[${i}]`,blocks:[{kind:"list",title:"Constructs measured",value:m.constructsMeasured},{kind:"text",title:"Response format",value:m.responseFormat},{kind:"text",title:"Item count",value:m.itemCount},{kind:"text",title:"Timeframe",value:m.timeframe},{kind:"text",title:"Reproduction",value:m.reproductionStatus},{kind:"text",title:"Clinical interpretation",value:m.clinicalInterpretationRequired?"Required":"Not specified"}]}));});panel.appendChild(list);return panel;
  }

  function renderPstVisuals(content, basePath) {
    const panel=makePstPanel("visuals","Visuals");panel.appendChild(makePstHeading("Visual asset inventory","Review placeholders, inclusion decisions, filenames, and replacement status."));
    const controls=document.createElement("div");controls.className="pst-filter-row";const input=document.createElement("input");input.type="search";input.placeholder="Filter visuals";const select=document.createElement("select");select.innerHTML='<option value="">All decisions</option><option value="include">Include</option><option value="optional">Optional</option><option value="pending">Pending</option><option value="omit">Omit</option>';controls.append(input,select);panel.appendChild(controls);
    const list=document.createElement("div");list.className="pst-card-list";panel.appendChild(list);
    const render=()=>{const q=input.value.trim().toLowerCase(),f=select.value;list.innerHTML="";pstArray(content.visualAssets).forEach((v,i)=>{if(!isPlainObject(v))return;if(q&&!JSON.stringify(v).toLowerCase().includes(q))return;if(f&&v.inclusionDecision!==f)return;const row=document.createElement("div");row.className="pst-visual-row";const body=document.createElement("div");const title=document.createElement("strong");title.textContent=v.title||v.graphicId;const detail=document.createElement("span");detail.textContent=[v.visualType,v.status,v.suggestedFilename].filter(Boolean).join(" · ");body.append(title,detail);const actions=document.createElement("div");const status=document.createElement("span");status.textContent=v.inclusionDecision||"pending";actions.appendChild(status);const source=makePstSourceButton(`${basePath}.visualAssets[${i}]`);if(source)actions.appendChild(source);row.append(body,actions);list.appendChild(row);});if(!list.children.length){const empty=document.createElement("div");empty.className="catalog-empty";empty.textContent="No visuals match.";list.appendChild(empty);}};input.addEventListener("input",render);select.addEventListener("change",render);render();return panel;
  }

  function renderPstCoverage(content, basePath) {
    const panel=makePstPanel("coverage","Coverage");panel.appendChild(makePstHeading("Source and production coverage","Source capture is separate from editorial and publication completion."));const source=pstObject(content.sourcePacket),coverage=pstObject(content.coverage),editorial=pstObject(content.editorialModel);const grid=document.createElement("div");grid.className="pst-summary-grid";appendPstTextBlock(grid,"Source format",source.sourceFormat);appendPstTextBlock(grid,"Estimated pages",source.estimatedPageCount);appendPstTextBlock(grid,"Reviewed through",source.lastReviewedPage);appendPstTextBlock(grid,"Coverage confidence",source.coverageConfidence);appendPstTextBlock(grid,"Core curriculum",coverage.coreCurriculumStatus);appendPstTextBlock(grid,"Optional handouts",coverage.optionalHandoutsStatus);appendPstTextBlock(grid,"Measures",coverage.measuresStatus);appendPstTextBlock(grid,"Visual review",coverage.visualReviewStatus);appendPstTextBlock(grid,"Editorial integration",coverage.editorialIntegrationStatus);appendPstTextBlock(grid,"Sidecar",coverage.sidecarStatus);appendPstTextBlock(grid,"HTML",coverage.htmlStatus);appendPstTextBlock(grid,"Master format",editorial.masterFormat);appendPstListBlock(grid,"Known gaps",coverage.knownGaps);appendPstListBlock(grid,"Non-blocking gaps",coverage.nonBlockingGaps);appendPstListBlock(grid,"Remaining work",coverage.remainingWork);panel.appendChild(grid);const sourceButton=makePstSourceButton(`${basePath}.coverage`);if(sourceButton)panel.appendChild(sourceButton);return panel;
  }

  function renderPstSpPreview() {
    const model=state.previewModel, field=model?.pstSpContent, content=pstObject(field?.value), basePath=field?.path||pathJoin(model?.basePath||"","pstSpContent");
    els.previewFileName.textContent=state.currentFile?.path||"PST-SP sidecar";els.previewPanelHeading.textContent="PST-SP study guide";els.previewContent.innerHTML="";
    const article=document.createElement("article");article.className="preview-document";const hero=document.createElement("header");hero.className="preview-hero";const eyebrow=document.createElement("p");eyebrow.className="preview-eyebrow";eyebrow.textContent="PST-SP structured preview";const title=document.createElement("h2");title.className="preview-title";title.textContent=compactText(model.title?.value,300)||"PST-SP Study Guide";hero.append(eyebrow,title);if(model.subtitle){const subtitle=document.createElement("p");subtitle.className="preview-subtitle";subtitle.textContent=compactText(model.subtitle.value,500);hero.appendChild(subtitle);}const meta=document.createElement("div");meta.className="preview-meta";appendMetaChip(meta,"Type",model.documentType);appendMetaChip(meta,"Date",model.date);appendMetaChip(meta,"Schema",model.schemaVersion);if(meta.children.length)hero.appendChild(meta);article.appendChild(hero);if(model.summary){const summary=document.createElement("p");summary.className="preview-summary";summary.textContent=compactText(model.summary.value,1800);article.appendChild(summary);}
    const shell=document.createElement("div");shell.className="pst-preview-sections";const tabs=[["overview","Overview"],["toolkits","Toolkits"],["exercises","Exercises"],["measures","Measures"],["visuals","Visuals"],["coverage","Coverage"]];const tabBar=document.createElement("div");tabBar.className="pst-tabs";tabBar.setAttribute("role","tablist");const panels=new Map([["overview",renderPstOverview(content,model)],["toolkits",renderPstToolkits(content,basePath)],["exercises",renderPstExercises(content,basePath)],["measures",renderPstMeasures(content,basePath)],["visuals",renderPstVisuals(content,basePath)],["coverage",renderPstCoverage(content,basePath)]]);const activate=id=>{tabBar.querySelectorAll(".pst-tab").forEach(b=>b.setAttribute("aria-selected",String(b.dataset.tabId===id)));for(const [key,panel] of panels)panel.hidden=key!==id;localStorage.setItem("jsonExplorerPstSpTab",id);};for(const [id,label] of tabs){const b=document.createElement("button");b.type="button";b.className="pst-tab";b.dataset.tabId=id;b.textContent=label;b.addEventListener("click",()=>activate(id));tabBar.appendChild(b);}shell.appendChild(tabBar);for(const panel of panels.values())shell.appendChild(panel);article.appendChild(shell);const footer=document.createElement("footer");footer.className="preview-footer";footer.textContent="Use JSON buttons to reveal authoritative records in Tree view.";article.appendChild(footer);els.previewContent.appendChild(article);const saved=localStorage.getItem("jsonExplorerPstSpTab");activate(panels.has(saved)?saved:"overview");
  }
