...
The repository also includes a generated physical directory inventory:
...
- list the folders and files that physically exist in the repository;
- provide a quick whole-repository orientation without requiring a manual filesystem walkthrough;
- make the current physical structure easy to compare with the intended semantic architecture;
- support documentation and maintenance work by exposing existing paths before changes are proposed.

Regenerate it from the repository root with:

```text
py build_repository_tree.py
```
...
The generated tree is deliberately descriptive rather than interpretive. It answers **what exists and where it lives**. It does not determine project membership, system boundaries, authority, current/canonical status, generators, consumers, or other semantic relationships.

Those higher-level meanings belong in this README, subsystem documentation, the Klinswork Resource Registry, canonical project/system records, and the planned `repository-manifest.json`.
...
The repository manifest should be more than a generated directory tree. The physical tree already exists as `repository-tree.txt`; the repository manifest should add semantic relationships, authority, roles, and architectural meaning rather than duplicate that file.
...
```text
1. Read README.md.
2. Read repository-tree.txt when a physical repository map is useful.
3. Read repository-manifest.json when available for the semantic repository map.
4. Identify the subsystem involved.
5. Follow that subsystem's read-first documentation.
6. Inspect current files and versions before proposing changes.
7. Determine which artifacts are source, generated, aggregate, companion, or published.
8. Determine whether the work changes any generator/consumer relationships.
9. Update durable documentation when new architectural facts are learned.
```
